	  ...
	  initThreeJS() {
		...
		instance
		  .post(
			...
			{
			  responseType: "arraybuffer", // 确保以ArrayBuffer形式接收响应数据
			}
		  )
		  .then(async (response) => {
			// 处理响应...
			const loader = new STLLoader();
			const geometry = loader.parse(response.data); // 使用parse方法而不是load
		  ...
		  const fileloader = new THREE.FileLoader();
			...
			instance.post("/api/user/download_swc", {
				// 你的请求体
				patient_id: 2,
			    teeth_num: 1,
			  })
			  .then((response) => {
				...
				// 假设response.data就是你的SWC格式数据
				const swcData = response.data;
				console.log(swcData + "SWCDATA")
